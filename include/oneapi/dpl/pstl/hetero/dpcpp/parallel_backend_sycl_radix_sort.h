        ...
        for (::std::uint32_t __radix_iter = 0; __radix_iter < __radix_iters; ++__radix_iter)
        {
            ...
            if (__radix_iter % 2 == 0)
                ...
                __event = __parallel_radix_sort_iteration<__radix_bits, __is_ascending, /*even=*/false>::submit(
                    ::std::forward<_ExecutionPolicy>(__exec), __segments, __radix_iter,
                    __out_rng, ::std::forward<_Range>(__in_rng), __tmp_buf, __event);

            // TODO: since reassign to __event does not work, we have to make explicit wait on the event
            explicit_wait_if<::std::is_pointer<decltype(__in_rng.begin())>::value>{}(__event);
        }
    }
