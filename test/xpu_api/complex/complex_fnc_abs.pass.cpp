...
protected:
    ...
    void test_abs()
    {
        ...
        EXPECT_TRUE_EE(errorEngine, abs_res == abs_res_expected, "Wrong result if dpl::abs(dpl::complex<T>()) function");
    }
    ...
    void test_edges()
    {
        ...
        for (unsigned i = 0; i < N; ++i)
        {
            ...
            const auto abs_res = dpl::abs(cv);
            const auto abs_res_expected = ::std::abs(cv);
            EXPECT_TRUE_EE(errorEngine, abs_res == abs_res_expected, "Wrong result if dpl::abs(dpl::complex<T>()) function #1");
        }
    }
