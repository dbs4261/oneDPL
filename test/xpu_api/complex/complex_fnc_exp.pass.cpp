...
public:
    ...
    {
        test_exp<float>("Wrong result if dpl::exp(dpl::complex<T>()) function (float)");

        // Sometimes device, on which SYCL::queue work, may not support double type
        TestUtils::invoke_test_if<IsSupportedDouble>()([&](){ test_exp<double>("Wrong result if dpl::exp(dpl::complex<T>()) function (double)"); });
        ...
        TestUtils::invoke_test_if<IsSupportedLongDouble>()([&](){ test_exp<long double>("Wrong result if dpl::exp(dpl::complex<T>()) function (long double)"); });
    }
